...
const LastProductDetail = () => {
  ...
  useEffect(() => {
    if (!isNewProduct) {
      const found = lastProductList.find((p) => p._id === id);
      if (found) {
        setProduct(found);
        setEditData(found);
  ...
  }, [id, isNewProduct, lastProductList, url]);
  ...
  const handleSaveChanges = async () => {
    try {
      ...
      if (isNewProduct) {
        await addLastProduct(formData);
        successToast(" Product added successfully!");
        fetchLastProducts();
        navigate("/homepage");
      } else {
        await updateLastProduct(product._id, formData);
    ...
    <div className="product-detail">
      ...
      <div className="product-detail-info">
        ...
        <div className="product-actions">
          <button className="save-btn last-btn" onClick={handleSaveChanges}>
            {isNewProduct ? "Add Product" : "Save Changes"}
          </button>
          {!isNewProduct && (
            <>
              <button
                className="discard-btn last-btn"
