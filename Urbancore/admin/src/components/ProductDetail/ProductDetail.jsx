...
const ProductDetail = () => {
  ...
  // Default template for a new product
  ...
  // Fetch product if editing existing
  useEffect(() => {
    if (!isNewProduct) {
  ...
  }, [id, isNewProduct, productList, url]);
          ...
          >
            ...
            <input
              ...
              onChange={(e) => {
                ...
                // If comma typed on mobile (will appear in value, not as key)
                if (val.includes(",")) {
                  const parts = val
                    .split(",")
                    .map((v) => v.trim())
                  ...
                  parts.forEach((tag) => {
                    if (tag && !newKeywords.includes(tag))
              ...
              onKeyDown={(e) => {
                // Handle Enter key across devices
                if (e.key === "Enter") {
                  e.preventDefault();
              ...
              onBlur={() => {
                // Add tag when user finishes typing and taps away (mobile-friendly)
